...
def getMyDataAvg():
    ...
    for mask in get_mask_subdirs_os2(f'C:/Users/Alexs/PyCharmMiscProject/pythonProject7/object_tracking/object_tracking/samMooooolq/segmentAnything'):
        ...
        synteticData = generateSynteticData(len(dataset))
        allData.append(synteticData)
        #allData.append(dataset)

        print("Real data", dataset['x'][0])
        print("SynteticData", synteticData['x'][0], synteticData['dx'][0])
        ...
        test_dxdt = torch.cat((test_dxdt, torch.tensor(test_dataset['dx'], requires_grad=True, dtype=torch.float32)), dim=0)
